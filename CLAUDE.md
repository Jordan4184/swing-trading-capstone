...
- **Features**: absolute (multi-horizon returns, vol, RSI, BB%, volume ratio, SPY excess return). A rank-feature variant lives at `src/features.py::build_features_ranked` as a documented research alternative — tested 2026-05-12, lifted walk-forward AUC by +1.19pp but degraded realized top-2 Sharpe; **kept production on absolute features**.
...
- **Current production**: `rf_v1.joblib` = absolute-feature model (original). `rf_v3.joblib` is a rank-feature model trained 2026-05-12 as an experiment; it was promoted briefly and then rolled back when the realized backtest Sharpe degraded (see `feature_ablation.json` for the research record). `rf_v1_pre_rank.joblib` is a snapshot of the production model at the moment of the rollback — currently identical to `rf_v1.joblib`.
...
**Interpretability**: `src/explain.py` precomputes SHAP values (TreeExplainer, exact) for every prediction and persists to `results/shap_values.parquet`. `src/failure_modes.py` hand-picks 3 of v2's worst basket returns and assembles structured case studies — date, picks, SHAP contributions per pick, realized return, and a one-line "what I'd change." Both feed the backend: `/api/explain/{ticker}/{date}` powers the ConvictionLedger strip under the Decision Card; `/api/failure-modes` powers the case-study cards on the Evaluation page.

**3D SHAP Interaction Surface**: `src/shap_surface.py` runs `TreeExplainer.shap_interaction_values` across all 17.8K predictions, picks the off-diagonal feature pair with the largest mean |Φ_ij| (currently `volatility_20d × volatility_60d`), bins observed values into a 20×20 grid of mean Φ_ij, and pins the 6 failure-case picks at their actual `(x, y, Φ_ij)` coords. Persisted to `results/shap_surface.json`. Frontend `ShapInteractionSurface.tsx` renders via `react-plotly.js` + `plotly.js-gl3d-dist-min` (dynamic-imported with `ssr: false` — plotly touches `window` at import time and would crash SSR otherwise). The surface is rendered as piecewise-constant tiles with gaps for empty bins because Random Forest is a step function; smoothing would be epistemically dishonest. Mounted on `/evaluation` directly above the Failure-Mode Case Studies section so the pinned cases tie back visually.

**Robustness layer (phase 5)**: two analytical artifacts that pre-empt the panel-chair's hardest questions.
- `src/null_test.py` runs a vol-targeted SPY benchmark with identical risk-layer rules (regime gate, rebalance cadence, cost model) — no model, no ranker — then a paired bootstrap on the per-rebalance Sharpe difference. Persisted to `results/null_test.json`. Current verdict: strategy Sharpe 1.09 vs vol-targeted SPY 0.33, paired p_two_sided = 0.029 — the alpha is statistically distinguishable from "vol-targeting carried it." Exposed at `/api/null-test`, rendered on `/evaluation` with overlaid equity curves and a 4-stat verdict block.
- `src/universe_robustness.py` re-runs the walk-forward CV + top-2 backtest on two alternative 11-name universes constructed away from megacap-tech concentration. Persisted to `results/universe_robustness.json`. Current verdict (FAIL the optimistic bar): headline Sharpe 1.00 collapses to 0.33 (diversified rotation) and 0.08 (different S&P 100 draw). The strategy's edge is concentrated in the headline universe — this is the honest finding that lets the candidate own the survivorship critique. Exposed at `/api/universe-robustness`, rendered on `/evaluation` with a per-universe table and a "Limitations" callout (not a strict PIT reconstruction; would need CRSP / Sharadar for that).
